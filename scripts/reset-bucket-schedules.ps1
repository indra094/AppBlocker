...
if ((-not $BucketId -or $BucketId.Count -eq 0) -and (-not $BucketName -or $BucketName.Count -eq 0)) {
    throw "Provide -BucketId and/or -BucketName. Example: .\scripts\reset-bucket-schedules.ps1 -BucketId 3,5 or .\scripts\reset-bucket-schedules.ps1 -BucketName Social,Games"
}
...
$releaseAction = "com.indrajeet.appblocker.action.RESET_BUCKET_SCHEDULES"
...
if ($text -match "result=(\d+)") {
    ...
    if ($resultCode -ne 0) {
        throw "Bucket timing reset failed. See broadcast output above."
    }
}
