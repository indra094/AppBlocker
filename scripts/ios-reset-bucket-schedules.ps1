...
if ((-not $BucketId -or $BucketId.Count -eq 0) -and (-not $BucketName -or $BucketName.Count -eq 0)) {
    throw "Provide -BucketId and/or -BucketName. Example: .\scripts\ios-reset-bucket-schedules.ps1 -FilePath .\AppBlocker-buckets.json -BucketName Social,Games"
}
...
$resetBuckets = New-Object System.Collections.Generic.List[string]
...
foreach ($bucket in $buckets) {
    ...
    if (-not ($matchesId -or $matchesName)) {
        continue
    }

    $removedCount = @($bucket.schedules).Count
    $bucket.schedules = @()
    $resetBuckets.Add("$currentBucketName ($currentBucketId, removed $removedCount schedule(s))") | Out-Null
}

ConvertTo-Json -InputObject @($buckets) -Depth 100 |
    Set-Content -LiteralPath $FilePath -Encoding utf8

if ($resetBuckets.Count -eq 0) {
    Write-Host "Reset timings for 0 buckets."
} else {
    Write-Host "Reset timings for $($resetBuckets.Count) bucket(s): $($resetBuckets -join ', ')"
}
